...
export default function Participants() {
  ...
  const { data: participants = [], isLoading } = useQuery({
    ...
    queryFn: () => cita.participants.list('-created_date', 500),
  });
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: async ({ ids, data }) => {
      await Promise.all(ids.map(id => cita.participants.update(id, data)));
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href="/intake/new" className="shrink-0">
              ...
              <TableBody>
                {filtered.map(p => (
                  ...
                  >
                    ...
                    <TableCell className="font-medium" onClick={() => router.push(`/participants/${p.id}`)}>{p.first_name} {p.last_name}</TableCell>
                    <TableCell className="text-muted-foreground text-sm" onClick={() => router.push(`/participants/${p.id}`)}>{p.email}</TableCell>
                    <TableCell className="text-muted-foreground text-sm hidden lg:table-cell" onClick={() => router.push(`/participants/${p.id}`)}>{p.phone || '—'}</TableCell>
                    <TableCell onClick={() => router.push(`/participants/${p.id}`)}><StageBadge stage={p.stage} /></TableCell>
                    <TableCell onClick={() => router.push(`/participants/${p.id}`)}><StatusBadge status={p.status} /></TableCell>
                    <TableCell className="text-muted-foreground text-sm hidden lg:table-cell" onClick={() => router.push(`/participants/${p.id}`)}>
            ...
            >
              <div className="flex items-start gap-3">
                <Checkbox
                  ...
                  className="mt-0.5"
                />
                <div className="flex-1 min-w-0" onClick={() => router.push(`/participants/${p.id}`)}>
